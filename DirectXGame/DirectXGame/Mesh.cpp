...
Mesh::Mesh(const wchar_t* full_path) : Resource(full_path)
{
	...
	m_vertex_buffer = GraphicsEngine::Get()->CreateVertexBuffer(&list_vertices[0], sizeof(VertexMesh),
		(UINT)list_vertices.size(), shader_byte_code, (UINT)size_shader);

	m_index_buffer = GraphicsEngine::Get()->CreateIndexBuffer(&list_indices[0], (UINT)list_indices.size());
}
